...
var argv = require('optimist').
        ...
        default('outputdir', '.').
        argv,
    pagedown = require('pagedown'),//用pagedown转义markdown
...
argv._.forEach(function(md_path) {
    ...
    levels = {}; nextId = 0; toc = [];
    output = '<div class="page-header bs-docs-header" id="top">' +
            '<h1>' + tags.title  + '</h1>' +
            '<span>' + tags.subtitle + '</span>' + 
            '</div>';
    output += converter.makeHtml(md);

    // Add table of contents
    tocHtml += '<div class="col-md-3 bs-docs-sidebar affix-top"><ul class="nav bs-docs-sidenav bs-docs-trss" data-spy="affix">';
    toc.forEach(function(entry) {
        tocHtml += '<li><a href="#' + entry.id + '">' + entry.levelStr + entry.title + '</a></li>';
    });
    tocHtml += '<li><a href="#top" id="back-to-top">回到顶部</a></li>';
    tocHtml += '</ul></div><div class="col-md-9 bs-docs-content">';

    // Bootstrap-fy
    output = 
        top_part.replace(/\{\{title\}\}/, tags.title === "TITLE HERE" ? "" : tags.title) +
        tocHtml +
        output +
        bottom_part;
